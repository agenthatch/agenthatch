...
<!-- TODO: 录制终端演示 GIF，展示完整流水线：
     agenthatch init → skills add → hatch → run
     推荐 asciinema 或 terminalizer -->
...
| **确定性** | LLM 每次解读不同 | 同一份 SKILL.md → 同一份 Agent 二进制 |
...
6 个专用 AI Harness 并发运行，每个有独立的人格、温度和模型层级：

| Harness | 职责 | 模型 | 温度 |
|---|---|---|---|
| **A — 身份** | 从 frontmatter 提取名称、版本、描述 | 小模型 | 0.1 |
| **B — 意图** | 推断触发短语和用户意图 | 小模型 | 0.5 |
| **C — 接口** | 设计工具签名、参数和返回类型 | 大模型 | 0.5 |
| **D — 基座** | 检测运行时基类和指令结构 | 大模型 | 0.3 |
| **E — 装配** | 交叉校验所有 Harness 输出，生成 AHSSPEC | 小模型 | 0.2 |
| **F — MCP** | 检测并配置 MCP 服务器连接 | 中等模型 | 0.3 |
...
<!-- TODO: 搭建文档站（MkDocs Material 或 Mintlify） -->
