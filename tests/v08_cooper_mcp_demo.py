# ruff: noqa: E501
"""v0.8 Cooper MCP Agent Quality Review — deepseek-v4-pro with thinking.

Real multi-turn conversation with Cooper Agent reading a Cooper knowledge page.
URL: https://cooper.didichuxing.com/knowledge/2206696011130/2207552153421
Params: knowledgeId=2206696011130, resourceId=2207552153421, appId=2 for knowledge
"""
import sys
...
def main():
    # ─── COOPER with deepseek-chat (baseline) ────────────────────────
    print("\n" + "="*60)
    print("PHASE 1: Cooper Agent with deepseek-chat (BASELINE)")
    print("="*60)
    coop = load_agent("cooper", model="deepseek-chat")

    chat(coop, "C1-CHAT",
         "请读取Cooper知识库页面，knowledgeId=2206696011130，resourceId=2207552153421。"
         "这个页面的标题和主要内容是什么？请简要总结。")

    # Clean up session for next phase
    coop._checkpoint_mgr = None

    # ─── COOPER with deepseek-v4-pro (thinking enabled) ──────────────
    print("\n" + "="*60)
    print("PHASE 2: Cooper Agent with deepseek-v4-pro (DEEP THINKING)")
    print("="*60)
    coop_v4 = load_agent("cooper", model="deepseek-v4-pro")

    chat(coop_v4, "C2-T1",
         "请读取Cooper知识库页面，knowledgeId=2206696011130，resourceId=2207552153421。"
         "我需要你分析这个页面的结构——有什么表格吗？有几级标题？主要内容分为几块？")
    ...
    print("="*60)
    chat(coop_v4, "C3-T1",
         "列出这个Cooper知识库（knowledgeId=2206696011130）的目录结构。"
         "有哪些子目录/子页面？")
    ...
    print("\n" + "="*60)
    print("ALL COOPER MCP DEMOS COMPLETE")
    print("="*60)
